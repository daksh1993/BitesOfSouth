...
class _AnalysisScreenState extends State<AnalysisScreen> {
  ...
  Future<Uint8List> _generatePdf(Map<String, dynamic> data) async {
    ...
    final logoBytes = await _fetchLogo();
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Text(
            'Analysis Report for ${_startDate != null ? DateFormat('dd/MM/yyyy').format(_startDate!) : 'All Time'} - ${_endDate != null ? DateFormat('dd/MM/yyyy').format(_endDate!) : 'Now'}',
          ...
          if (selectedAnalyses.contains('Top Selling Item')) ...[
            ...
            if (itemQuantities.isNotEmpty) ...[
              pw.Text(
                  'Item: ${menuItems[itemQuantities.entries.reduce((a, b) => a.value > b.value ? a : b).key]?['title'] ?? 'Unknown'}',
                  style: pw.TextStyle(font: ttf)),
              pw.Text(
                  'Revenue: ${currencyFormat.format(itemRevenues[itemQuantities.entries.reduce((a, b) => a.value > b.value ? a : b).key] ?? 0)}',
                  style: pw.TextStyle(font: ttf)),
              pw.Text(
                  'Profit: ${currencyFormat.format(itemProfits[itemQuantities.entries.reduce((a, b) => a.value > b.value ? a : b).key] ?? 0)}',
                  style: pw.TextStyle(font: ttf)),
            ],
            pw.SizedBox(height: 10),
          ],
          if (selectedAnalyses.contains('Highest Revenue Item')) ...[
            ...
            if (itemRevenues.isNotEmpty) ...[
              pw.Text(
                  'Item: ${menuItems[itemRevenues.entries.reduce((a, b) => a.value > b.value ? a : b).key]?['title'] ?? 'Unknown'}',
                  style: pw.TextStyle(font: ttf)),
              pw.Text(
                  'Revenue: ${currencyFormat.format(itemRevenues.entries.reduce((a, b) => a.value > b.value ? a : b).value)}',
                  style: pw.TextStyle(font: ttf)),
            ],
            pw.SizedBox(height: 10),
          ],
          ...
          if (selectedAnalyses.contains('Trending Item')) ...[
            pw.Text('Trending Item',
                style: pw.TextStyle(
                    fontSize: 18, fontWeight: pw.FontWeight.bold, font: ttf)),
            pw.Text('Most popular item based on sales volume.',
                style: pw.TextStyle(font: ttf)),
            if (trendingItems.isNotEmpty) ...[
              ...
              pw.Text('No trending items available.',
                  style: pw.TextStyle(font: ttf)),
            ],
            pw.SizedBox(height: 10),
          ],
          if (selectedAnalyses.contains('Total Items Sold')) ...[
            pw.Text('Total Items Sold',
                style: pw.TextStyle(
                    fontSize: 18, fontWeight: pw.FontWeight.bold, font: ttf)),
            pw.Text('Total number of items sold.',
                style: pw.TextStyle(font: ttf)),
          ...
          if (selectedAnalyses.contains('Avg Revenue per Item')) ...[
            pw.Text('Average Revenue per Item',
                style: pw.TextStyle(
                    fontSize: 18, fontWeight: pw.FontWeight.bold, font: ttf)),
            pw.Text('Average revenue generated per item.',
                style: pw.TextStyle(font: ttf)),
            pw.Text(
          ...
          if (selectedAnalyses.contains('Top 3 Items Revenue Share')) ...[
            pw.Text('Top 3 Items Revenue Share',
                style: pw.TextStyle(
                    fontSize: 18, fontWeight: pw.FontWeight.bold, font: ttf)),
            pw.Text(
                'Revenue contribution of the top 3 revenue-generating items.',
                style: pw.TextStyle(font: ttf)),
            if (itemRevenues.isNotEmpty) ...[
              pw.Text(
                  'Total Revenue: ${currencyFormat.format(sortedRevenues.take(3).fold(0.0, (sum, e) => sum + e.value))}',
                  style: pw.TextStyle(font: ttf)),
              ...(sortedRevenues.take(3).map((e) => pw.Text(
                  '${menuItems[e.key]?['title'] ?? 'Unknown'}: ${currencyFormat.format(e.value)}',
                  style: pw.TextStyle(font: ttf)))),
            ],
            pw.SizedBox(height: 10),
          ],
  ...
  Future<void> _savePdf(Uint8List pdfBytes) async {
    try {
      ...
      final file = File(
          '${directory.path}/Analysis_Report_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf');
  ...
  void _showPdfOptions(Map<String, dynamic> data) async {
    final pdfBytes = await _generatePdf(data);
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('PDF Options'),
        content: const Text('Choose an action for the generated PDF:'),
        actions: [
          // TextButton(
          //   onPressed: () async {
      ...
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Colors.grey.shade100, Colors.white], // Fixed: Two colors
  ...
  Widget _buildNetSalesCard(
    ...
    return Card(
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Colors.blue.shade50, Colors.white], // Fixed: Two colors
  ...
  Widget _buildNetProfitCard(
    ...
    return Card(
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Colors.green.shade50, Colors.white], // Fixed: Two colors
            begin: Alignment.topLeft,
            end: Alignment.bottomRight,
          ),
          borderRadius: BorderRadius.circular(20),
  ...
  Widget _buildAnalysisCard(
    ...
    return Card(
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [Colors.white, Colors.grey.shade50], // Fixed: Two colors
            begin: Alignment.topLeft,
            end: Alignment.bottomRight,
          ),
          borderRadius: BorderRadius.circular(20),
